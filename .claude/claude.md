...
## Related Repository: sjlc-private (Backend)

This frontend is a **read-only output** of the backend system. All content is generated by
`sjlc-private`, which runs as the `sjlc-admin` Docker container on the Synology NAS.

| What | Where |
|------|-------|
| Backend repo | `…/projects/sjlc/backend/` on the NAS |
| Admin UI | Flask app at `backend/gui/`, served via the Cloudflare tunnel (admin.sjlcdailybible.org) |
| Content generation | `backend/src/day_processor.py`, `backend/src/content_manager.py` |
| Database | SQLite at `backend/data/database/sjlc_readings.db` (git-untracked; Synology backup) |

**Content Flow:**
```
sjlc-private (Backend)              sjlc-readings-frontend (This Repo)
├── Admin imports readings    ──►   (nothing - backend only)
├── Admin triggers generation ──►   years/{year}/daily_readings/*.html
├── day_processor.py runs     ──►   years/{year}/images/*.webp
└── readings_json_manager.py  ──►   years/{year}/readings.json
```

**Key Rule:** Never manually edit files in `years/` - they are generated by the backend and will be overwritten.

**Environment:** Everything runs on the **Synology NAS**, accessed via **SSH in VSCode** for code and
the **Cloudflare tunnel** for the admin app — there is no PC copy. This clone is a real git repo
(SSH origin + deploy key): design edits here get committed and pushed, and GitHub Pages redeploys
https://sjlcdailybible.org. See the root [CLAUDE.md](../../CLAUDE.md) for NAS layout and ops. Both
repos live side by side under `…/projects/sjlc/`:
- Frontend: `…/projects/sjlc/frontend/` (this repo — displays content)
- Backend: `…/projects/sjlc/backend/` (generates it)

Always check BOTH when searching for code.
