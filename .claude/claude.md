...
This frontend is a **read-only output** of the backend system. All content is generated by `sjlc-private`.

| What | Where |
|------|-------|
| Backend repo | `c:\pyApps\sjlc-private` |
| Admin UI | Flask app at `gui/` |
| Content generation | `src/day_processor.py`, `src/content_manager.py` |
| Database | SQLite at `data/sjlc_readings.db` |
...
**Workspace Note:** This repo is part of the `sjlc-daily-readings.code-workspace` alongside `sjlc-private`. Always check BOTH repos when searching for code - frontend displays content, backend generates it. Paths:
- Frontend: `c:\pyApps\sjlc-readings-frontend`
- Backend: `c:\pyApps\sjlc-private`
